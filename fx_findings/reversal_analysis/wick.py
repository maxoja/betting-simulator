...
def run():
    ...
    df = utils.slice_frame2(df, 2*utils.annual_bars(TIMEFRAME))
    data_size = len(df)
    ...
    for entry_min_rise in thresh_range:
        ...
        # entry_idx = np.logical_and(entry_idx, rsi_5 < 53)
        ...
        plotting.plot_histogram(profits, title=f"{entry_min_rise} avg {utils.avg(profits):.02f} x {len(profits)}")
